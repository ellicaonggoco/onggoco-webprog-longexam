...
const navLinkClassName = ({ isActive }) =>
  [
    'rounded-full border-2 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.24em] transition focus:outline-none focus:ring-2 focus:ring-[#ffd600]',
    isActive
    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b-2 border-[#ffd600] bg-[#1a237e]/95 backdrop-blur shadow-lg">
      <div className="mx-auto max-w-6xl flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center min-w-0">
          <NavLink to="/" className="flex items-center gap-3 group">
            <span className="relative flex items-center justify-center h-11 w-11">
              <img src={logo} alt="BulldogEx" className="h-11 w-11 rounded-full border-2 border-[#ffd600] bg-white object-contain shadow-md group-hover:scale-105 transition" />
        ...
        <nav className="hidden md:flex flex-1 items-center justify-center gap-4">
          {links.map((link) => (
            <NavLink key={link.to} to={link.to} end={link.to === '/'} className={navLinkClassName}>
              <span className="tracking-wider text-base font-bold">{link.label}</span>
            </NavLink>
          ))}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-2 ml-4">
          <NavLink to="/auth/signin" className={({ isActive }) => [
            'rounded-full border-2 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.24em] transition focus:outline-none focus:ring-2 focus:ring-[#ffd600]',
            isActive
